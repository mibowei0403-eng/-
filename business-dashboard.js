...
function statusChip(status) {
  ...
  if (String(text).includes("丢失") || String(text).includes("异常")) tone = "danger";
...
function openDeviceForm(code) {
  ...
  form.reset();
  const device = findRawDevice(code);
  if (!device) return;
  form.elements.code.value = device.code;
  form.elements.codeDisplay.value = device.code;
  ["status", "cost", "rent", "collected", "rentedMonths", "depositFree", "paidBack", "spec"].forEach((key) => {
    if (form.elements[key]) form.elements[key].value = device[key] ?? "";
  });
...
function renderCommandCenter() {
  ...
  const activeDebtPayment = data.loans
    ...
    .reduce((sum, loan) => sum + Number(loan.monthlyPayment || 0), 0);
...
function renderOverview() {
  ...
  const risk = devices.filter((device) => ["丢失", "空置"].includes(device.status)).length;
  ...
  document.querySelector("#deviceStatusText").textContent = `${devices.filter((d) => d.status === "在租").length} 台在租 / ${devices.filter((d) => d.status === "空置").length} 台空置 / ${devices.filter((d) => d.status === "丢失").length} 台丢失`;
...
function renderRentReminders() {
  const reminders = rentLedgerRows()
    .map((row) => ({ order: row.order, date: row.dueDate, left: daysUntil(row.dueDate) }))
    .filter((item) => item.date)
    .sort((a, b) => a.left - b.left)
    .slice(0, 10);

  document.querySelector("#rentReminderList").innerHTML = reminders.map(({ order, date, left }) => {
    let tone = "good";
    let label = `${left}天后`;
    if (left < 0) {
      tone = "danger";
    ...
    return `
      <div class="reminder-row">
        <div>
          <strong>${order.deviceCode || "-"} / ${order.customer || "未填写客户"}</strong>
          <span>合同签订：${order.contractDate || order.startDate || "-"}，收租日：${formatDate(date)}</span>
        </div>
        <div class="reminder-actions">
          <span class="chip ${tone}">${label}</span>
          <button class="mini-button" type="button" data-collect-rent="${order.id}">收款</button>
        </div>
      </div>
    `;
...
function renderFinanceHealth() {
  ...
  const activeLoans = data.loans.filter((row) => Number(row.remainingPrincipal || 0) > 0);
  const activeLoanPrincipal = activeLoans.reduce((sum, row) => sum + Number(row.principal || 0), 0);
  const loanRemain = activeLoans.reduce((sum, row) => sum + Number(row.remainingPrincipal || 0), 0);
  const monthlyPayment = activeLoans.reduce((sum, row) => sum + Number(row.monthlyPayment || 0), 0);
  ...
  const zeroPaymentDebt = activeLoans
    .filter((row) => Number(row.monthlyPayment || 0) <= 0)
  ...
  document.querySelector("#financeLoanRemainText").textContent = `未结清 ${activeLoans.length} 笔，剩余本金 ${money(loanRemain)}`;
...
document.querySelector("#deviceForm").addEventListener("submit", async (event) => {
  ...
  const device = findRawDevice(values.code);
  if (!device) return;
...
loadData().catch((error) => {
  document.querySelector("#storageStatus").textContent = "读取失败";
  console.error(error);
});
